...
Atualmente, a maior dificuldade para medir o tempo de geração na GPU é a falta de um padrão nas extensões disponíveis em OpenGL. A extensão \textbf{GL\_EXT\_timer\_query}, por exemplo, só está disponível em placas NVidia, algo que anularia a possibilidade da execução deste trabalho em placas ATI.

A utilização de chamadas como \textbf{glFinish()} para sincronizar a CPU e a GPU e assim medir o tempo de geração dos terrenos poderia prejudicar a performance do sistema, já que pára a execução da CPU enquanto todos os comandos OpenGL não forem executados.
...
\begin{figure}[h]
	...
	\caption{\label{fig:texturas} Movimentação da câmera para um outro \emph{patch}.}
\end{figure}
