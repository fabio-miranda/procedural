...
\label{resultados}
Neste capítulo, será apresentado algumas imagens de terrenos gerados (Seção \ref{terrenosgerados}) e também algum testes executados (\ref{testes}).
...
A Figura \ref{fig:deslocamento} mostra o mesmo mapa de altura, mas agora com o deslocamento dos vértices no eixo \emph{z}.
\begin{figure}[H]
...
A Figura \ref{fig:blend} mostra agora a renderização da malha com cores para simular grama, pedras, neve, etc.
\begin{figure}[H]
...
Um vídeo demonstrando a navegação pelo terreno pode ser visto em \cite{youtube}.


\section{Testes de Desempenho}
\label{testes}
Alguns testes foram feitos para avaliar a velocidade de geração dos terrenos com a alteração de alguns parâmetros, utilizando tanto a GPU quanto a CPU. Eles foram executados em um \emph{Core 2 Duo E7400}, com 2GB de memória \emph{RAM} e placa de vídeo \emph{ATI Radeon HD 4850} com 512MB de memória \emph{RAM}, e \emph{driver} versão 8.612. As tabelas com os tempos e as conclusões dos testes são apresentadas a seguir.
