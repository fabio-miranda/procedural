...
\begin{figure}[H]
	\center{\includegraphics[width=0.5\linewidth]{img/bibliotecas.png}}
...
\subsection{Geração do Terreno}
\label{geracao}
Nesta seção, será abordado a implementação da geração de terrenos, tanto na \emph{GPU}, quanto na \emph{CPU}. Os dois têm, em comum, o algoritmo usado para a geração (\emph{Ridged multifractal noise}, descrito na Seção \ref{ridged}).

O terreno geral é dividide em terrenos menores (chamados \emph{patchs}), como mostro \emph{grid} da Figura \ref{fig:resultados:grid}:
...
Como o mapa de altura é gerado na \emph{GPU}, não há qualquer tipo de perda de desempenho com a transferência entre a memória RAM e a memória da placa de vídeo. Um aspecto importante é que, durante a geração do mapa de altura, os valores das normais de cada vértice também são calculados.



\section{Geração do Terreno na \emph{CPU}}
...
\subsubsection{Visualização do Terreno}
\label{visualizacao}
...
Na Figura \ref{fig:texturas} é possível notar o deslocamento dos mapas de textura quando a câmera move para o \emph{patchs} superior ao (1,1). Para que haja uma transição, uma matriz de translação, com valores iguais ao tamanho do \emph{patch}, é feita e multiplicada à matriz \emph{MODELVIEW}.

 Este método, possível somente para terrenos gerados na \emph{GPU}, diminuiu a necessidade de implementação de um algoritmo de nível de detalhe mais robusto. Além disso, como sabemos o número de vértices antecipadamente, a performance do aplicativo tem uma menor chance de sofrer quedas bruscas de rendimento.


\subsubsection{Visualização do Terreno Gerado na \emph{GPU}}
...
\subsection{\emph{Frames} por Segundo Durante Navegação}
\label{testeFPS}
\input{testeFPS}
